...
class MultiFileWriter:
    ...
    def __init__(self, base_dir, name):
        ...
        self._name = name
        self._file_gen = (open(self._base_dir / f'{name}_{i:02}.bin', 'wb')
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            if f_name not in self._open_files:
                # if f_name
                self._open_files[f_name] = open('/content/gdrive/MyDrive/postings_gcp/' + f_name, 'rb')
...
class InvertedIndex:
    ...
    -----------
      docs: dict mapping doc_id to list of tokens
        ...
        for doc_id, tokens in docs.items():
            self.docs_len(doc_id, tokens)
            self.add_doc(doc_id, tokens)

    def docs_len(self, doc_id, token):
        self.docs_len[doc_id] = len(list(token.split()))
    ...
    def read_posting_list(self, w):
        ...
        with closing(MultiFileReader()) as reader:
            ...
            b = reader.read(locs, self.df[w] * TUPLE_SIZE1)
            posting_list = []
            for i in range(self.df[w]):
                doc_id = int.from_bytes(b[i * TUPLE_SIZE1:i * TUPLE_SIZE1 + 4], 'big')
                tf = int.from_bytes(b[i * TUPLE_SIZE1 + 4:(i + 1) * TUPLE_SIZE1], 'big')
                posting_list.append((doc_id, tf))
            return posting_list
